...
def submit_form():
    ...
    if request.method == "POST":
        ...
        # ✅ WEBCAM BLOCK (ALAG LEVEL PAR)
        elif 'webcam_button' in request.form:
            vid_path = 0
            video_frames = cv2.VideoCapture(0, cv2.CAP_DSHOW)

            video_frames.set(cv2.CAP_PROP_BUFFERSIZE, 1)

            if not video_frames.isOpened():
                return 'Error: Cannot access webcam', 500
            else:
                frames_buffer.clear()
                return 'Webcam started successfully'
if __name__ == "__main__":
    #copy video from default dir to video uploads directory
    shutil.copy(VIDEO_PATH, app.config["VIDEO_UPLOADS"])
