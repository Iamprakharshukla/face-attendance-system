...
DATASET_PATH = r"C:\Users\prakh\Downloads\attendance_system-main\attendance_system-main\dataset"
# The path where dlib face encodings are stored
DLIB_FACE_ENCODING_PATH = os.path.join(BASE_DIR, 'data/gril_team_face_encodings.pkl')

# The path where face recognition report will be stored
REPORT_PATH = os.path.join(BASE_DIR, 'reports/inferred_faces.csv')

# Fles path where various supplementary files are stored
FILES_PATH = os.path.join(BASE_DIR, 'data/files/')

# The path of the demo video
VIDEO_PATH = os.path.join(BASE_DIR, 'data/files/vid.mp4')

# The path where the user video will be uploaded
VIDEO_UPLOAD_PATH = os.path.join(BASE_DIR, 'src/static/video/vid.mp4')

# The path where logs will be stored
LOG_FILE_PATH = os.path.join(BASE_DIR, 'logs/multicam_server.log')
